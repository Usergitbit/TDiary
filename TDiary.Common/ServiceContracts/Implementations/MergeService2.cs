...
{
    public class MergeService2 : IMergeService
    {
        ...
        private Queue<EventResolution> ResolveEvents(IEnumerable<Event> incomingEvents, IEnumerable<Event> outgoingEvents)
        {
            ...
            var localEventsResolutions = ResolveLocalEvents(incomingEventsResolutionState, outgoingEvents);
        ...
        private IncomingEventResolutionState ResolveRemoteEvents(IEnumerable<Event> incomingEvents)
        {
            ...
            foreach (var incomingEvent in incomingEvents)
            {
                result.EventResolutions.Add(new EventResolution
                {
                    ...
                    Reason = "Remote events must be played after undoing local unes."
                });
        ...
        private IEnumerable<EventResolution> ResolveLocalEvents(IncomingEventResolutionState incomingEventResolutionState, IEnumerable<Event> outgoingEvents)
        {
            ...
            foreach (var outgoingEvent in outgoingEvents)
            {
                ...
                if (!isAffectedByIncoming)
                {
                    result.Add(new EventResolution
                    {
                        ...
                        Reason = "Local event is eligible for push."
                    });
                }
                else
                {
                    var eventResolution = ResolveLocalEvent(outgoingEvent, incomingEventResolutionState);
        ...
        private EventResolution ResolveLocalEvent(Event outgoingEvent, IncomingEventResolutionState incomingEventResolutionState)
        {
            var eventResolution = new EventResolution
            {
                Event = outgoingEvent,
